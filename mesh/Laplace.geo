...
For i_spost In {0:n_spost-1}
    ...
    Transfinite Curve {l_coppa_p_sx} = 75 Using Progression 1.04;
    Transfinite Curve {l_coppa_p_dx} = 75 Using Progression 1.04;

    Transfinite Curve {semic_1} = 25 Using Progression 1.0;
    Transfinite Curve {semic_2} = 25 Using Progression 1.0;
	...
	// Physical Curve(71) = {b(0)};
	// Physical Curve(72) = {b(5)};

	b_soil() = Boundary{Surface{S_soil};};
	Printf("tags SOIL: ", b_soil()); // stampo tags physycal entities di S_soil
	// Physical Curve(73) = {b(0)};
	// Physical Curve(74) = {b(1)};
	
	i_physical_curve = 70;
	name_physical_curve = "Contorno";
	raccolta_curve = {b_air(0),b_air(5),b_soil(0),b_soil(1)};
